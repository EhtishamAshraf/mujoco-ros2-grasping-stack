...
// This class inherits from rclcpp::Node, meaning it becomes a ROS2 node and gains all ROS node functionality
...
public:
  ...
  void run()
  {
    ...
    auto cylinder_pose_side = cylinder_pose; // define a side pose with a little shift
    cylinder_pose_side.position.x += 0.1;

    // open the hand in the beginning if the hand service is available
    ...
    remove_collision_object("pick_cylinder"); // remove cylinder so moveit doesn't show collision between gripper and cylinder
    ...
    // Once object is grasped, return back to the Home position:
    rclcpp::sleep_for(std::chrono::seconds(1));
...
private:
  ...
  // Function to compute the cartesian path
  ...
  // remove collision objects from the scene based on 'object ID'
  void remove_collision_object(const std::string& object_id)
  {
    std::vector<std::string> object_ids; // Create a vector to hold the IDs of collision objects to remove
  ...
  void add_table_collision()
  {
    ...
    // Lambda function to create collision objects of Box shape
    ...
    // Lambda function to create collision objects of Sphere shape
    ...
    // Lambda function to create collision objects of Cylinder shape
    ...
    // Cylinder from MuJoCo: pos = (0.398, -0.348, 0.799) and size = (radius=0.025, half-height=0.075) -> height = 0.15
    collision_objects.push_back(
        make_cylinder("pick_cylinder", 0.15, 0.025, 0.398, -0.348, 0.799));

    // Ball from MuJoCo: pos = (0.7625, -0.335, 0.799), radius = 0.033
    collision_objects.push_back(
