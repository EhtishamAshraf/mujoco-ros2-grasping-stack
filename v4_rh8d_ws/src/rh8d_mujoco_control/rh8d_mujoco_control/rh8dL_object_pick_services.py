...
class RH8DHandServiceController(Node):
    def __init__(self):
        ...
        # ReentrantCallbackGroup() means callbacks in this group can run in parallel, even if one is already running.
        self.service_cb_group = ReentrantCallbackGroup() # for service calls
        self.timer_cb_group = ReentrantCallbackGroup()   # for periodic timer
        self.sub_cb_group = ReentrantCallbackGroup()     # for subscription callbacks
        ...
        # Publisher used to send actuator position commands to the RH8D position controller
        ...
        # Service to handle high-level hand commands
        ...
        # List of joint names in the order the controller will command
        ...
        self.range_threshold = 0.25 # RangerFinder threshold
        self.tip_force_threshold = 1.0 # Force threshold
        ...
        # order: [forearm, palm_axis, palm_l, thumb_axis, thumb_meta, index, middle, ring]
        self.current_command = [0.0] * 8

        self.thumb_axis_open = 1.57 # Default thumb rotation for fully open position
        ...
        # Maps gesture names to joint positions for high-level gesture commands.
        self.gesture_map = {
            'fully_open': {
        ...
        self.timer = self.create_timer(self.timer_period, self.step, callback_group=self.timer_cb_group)
    ...
    def publish_command(self, cmd_values):
        ...
        pt.time_from_start = Duration(sec=0, nanosec=int(self.timer_period * 1e9))
    ...
    def ramp_command_towards(self, target_positions, dt, freeze_mask=None):
        ...
        new_cmd[4] = move(4, target_positions[4], self.rate_thumb)
        new_cmd[5] = move(5, target_positions[5], self.rate_index)
        new_cmd[6] = move(6, target_positions[6], self.rate_middle)
        new_cmd[7] = move(7, target_positions[7], self.rate_ring_small)
    ...
    def is_grasp_contact_stable(self, finger_name: str) -> bool:
        ...
        # Maps finger names (thumb, index, etc.) to their sensor readings.
        tip_map = {
    ...
    # Resets all finger contact counters to zero.
    ...
    # Marks the current command as done and resets the hand state so it’s ready for the next command.
    ...
    # Make the Hand ready for the new command 
    def start_new_command(self, mode: str, gesture_name: str):
        # Prepares the system to start a new gesture. Resets command-related flags: busy, done, success, message.
        ...
        # When starting a grasp, ensure the previous grasp is unlocked and reset all finger contact counters so we can measure new stable contacts.
    ...
    # This is the service callback for the HandCommand service
    def handle_hand_command(self, request, response):
        mode = request.mode.strip().lower() # mode
        gesture_name = request.gesture_name.strip().lower() # gesture name

        # Acquire the state lock to safely read/update shared variables across multiple threads
        with self.state_lock:
            # Check if hand is busy
            if self.busy:
                response.success = False
                response.message = 'Hand is busy'
                return response
            
            # Check if mode is object, Only "open" and "grasp" are valid object commands
            if mode == 'object':
            ...
            # Check if mode is gesture, the gesture_name, must exist in gesture_map
            elif mode == 'gesture':
            ...
            self.start_new_command(mode, gesture_name) # start the new command
    ...
    # Function to gradually move the hand joints toward the fully open pose.
    ...
    # Similar to run_object_open, but for any predefined gesture.
    ...
    # Gradually closes the hand to grasp an object while checking finger contact sensors, and locks the grasp when enough fingers have stable contact
    def run_object_grasp(self):
        ...
        # If grasp is not locked then first latch the command
        if enough_contact and not self.grasp_locked:
            self.latch_current_grasp()

        # If grasp is locked, then close the hand with latched positions 
        ...
        # otherwise continue closing normally
        self.ramp_command_towards(target, self.timer_period, freeze_mask=freeze_mask)

    def step(self):
        with self.state_lock:
            # if state is not available, return
            if not self.have_state:
                return
            
            # If busy is False, the hand is idle, return
            if not self.busy:
                return

            # Copy current command information
        ...
        if active_mode == 'object':
            if active_command == 'open':
                with self.state_lock:
                    self.run_object_open() # Open the hand
            elif active_command == 'grasp':
                with self.state_lock:
                    if self.object_present(): # check if object is present
                        self.get_logger().info('Object detected in grasp range')
                        self.last_object_seen_time = self.get_clock().now()
                        self.run_object_grasp() # grasp the object
                    else:
